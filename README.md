...
LLM 只扩展智能搜索的查询词，不生成课程标准、code、匹配理由或硬筛选。模型超时、鉴权失败或返回结构异常时，API 仍返回确定性可信检索结果。发送到所配置模型服务前，服务端会去除可识别的邮箱、手机号、身份证号和带标签的姓名/学校/班级信息；请求正文和原始查询不会写入指标。智能检索使用独立的 AI 每分钟限流。自动去标识化不能替代数据最小化，请勿主动提交学生个人信息或其他敏感内容。
