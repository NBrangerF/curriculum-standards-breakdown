...
- `textbooks:h4g-theme-bridge-anchor-group-item-review-downstream-post-candidate-manual-review-confirmation-evidence-packet` 是这 15 条 bounded-source confirmation work items 的页文本证据 gate。当前 evidence items 为 15/15，text-extracted/ready-for-manual-review 为 15/15；page hint source 为 unit index override 4、unit index hint 9、PDF title search 2，sibling grade context 覆盖 H4G7+H4G8、H4G7+H4G9、H4G7+H4G8+H4G9。audit 使用 `--require-text`，仍不修改 editable decisions、不批准 bridge、不写正式数据、不进入 matcher/publication。
