...
- `textbooks:h4g-theme-bridge-remediation-recommendations` 是 subject-theme bridge remediation 的窄化决策 gate。它只允许把指定 needs-revision action family 转成 rejection，不新增 approval；当前第一轮只拒绝 110 条 English `Language in use` title-only bridge。
