...
| `scripts/textbooks/build_h4g_subject_theme_bridge_anchor_group_item_review_downstream_action_closure_candidates_after_post_confirmation.js` | 在旧 combined closure candidate 后吸收 15 条正文页确认完成的 post-candidate confirmation decisions；形成 149 条候选闭环，52 条 exact-anchor 行继续 pending，避免改写旧 combined 的前置语义。 |
| `scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_action_closure_candidates_after_post_confirmation.js` | 审计 after-post-confirmation closure candidate；确认 inherited combined=134、post-confirmation=15、candidate=149、pending=52，且非 post-confirmation rows 保持不变，official close/auto-close 仍为 0。 |
...
| `scripts/textbooks/build_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_review_packet.js` | 将 52 条 source-anchor exact rows 先压缩为 7 个 unit-level review groups，暴露同一单元/泛主题 fan-out 到多条 standards 的风险；只读不写 decisions。 |
| `scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_review_packet.js` | 审计 exact group review packet 精确覆盖 52 条 exact evidence rows，确认 group items 为 7、missing/unexpected=0、auto approval=0，仍不可作为 matcher/publication approval。 |
| `scripts/textbooks/build_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_review_decisions_template.js` | 从 7 个 unit-level exact group review items 生成可编辑 decisions template；每组默认 pending，用于判断回到 standard-level exact review、先拆组、拒绝过宽单元或补证据。 |
| `scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_review_decisions.js` | 审计 exact group review decisions 与 group packet 1:1 对齐；pending 默认合法，completed rows 必须补 reviewer metadata，且 group decision 仍不能直接批准任何 standard。 |
| `scripts/textbooks/build_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_review_recommendations.js` | 从 7 条 exact group review decisions 生成 recommendation-only 分流；建议 4 组先拆、2 组拒绝过宽/泛主题、1 组补更具体证据，0 组进入 standard-level ready。 |
| `scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_review_recommendations.js` | 审计 exact group review recommendations 与 decisions 1:1 对齐，重新计算推荐结果，确认 missing/extra=0、standard-level ready=0、auto approval=0。 |
| `scripts/textbooks/build_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_review_action_worklist.js` | 将 7 条 exact group recommendations 转成 7 个可执行 action work items；队列为 split 4、reject overbroad/generic 2、collect evidence 1，仍不批准任何 standard。 |
| `scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_review_action_worklist.js` | 审计 exact group action worklist 与 recommendations/decisions 1:1 对齐，确认 missing/extra=0、standard-level ready=0、auto approval=0。 |
| `scripts/textbooks/build_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_surface.js` | 将 4 个 split action groups 展开为 45 条 standard-level exact-anchor review rows，按 `standard_code+grade_band+unit_evidence_id+exact_anchor_evidence_packet_id` 粒度审阅。 |
| `scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_surface.js` | 审计 split review surface 精确覆盖 split action queue 的 45 条 exact evidence rows，确认 parent actions=4、missing/extra=0、auto approval=0。 |
| `scripts/textbooks/build_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_decisions_template.js` | 从 45 条 split review rows 生成可编辑 standard-level decisions template；每条默认 pending，只能记录 accept candidate/reject/more evidence/activity split，不直接批准 standard。 |
| `scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_decisions.js` | 审计 split review decisions 与 split surface 45/45 对齐，确认 pending=45、completed=0、missing/extra=0、auto approval=0。 |
| `scripts/textbooks/build_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_recommendations.js` | 从 45 条 split review decisions 生成 recommendation-only 分流；pending 保持 pending，completed decisions 才会映射到 accept/reject/more evidence/activity split 的后续 gate。 |
| `scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_recommendations.js` | 审计 split review recommendations 与 decisions 45/45 对齐，确认 missing/extra=0、auto approval=0，并防止 recommendation 被误当作 matcher/publication approval。 |
| `scripts/textbooks/build_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_packet.js` | 为 45 条 split review rows 生成只读 progression contrast packet；逐条并排 H4G7/H4G8/H4G9 同组 public standards，暴露 official standard 文本是否相同、sibling context 是否缺失，以及当前 split evidence 是否只是单年级孤岛。 |
| `scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_packet.js` | 重新从 split review surface 与 `public/data/by_subject` 复算 progression contrast packet，确认 expected/packet=45/45、missing/extra/mismatch=0、public write=0、auto approval=0。 |
| `scripts/textbooks/build_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_worklist.js` | 将 45 条 contrast rows 按 `progression_group_id` 聚合成 30 个 progression-level action work items；队列为 repair missing sibling context 3、prove current-grade-specific evidence 18、collect sibling-grade evidence 9。 |
| `scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_worklist.js` | 重新从 contrast packet 复算 progression action worklist，确认 expected/action=30/30、45/45 contrast rows 全覆盖、missing/extra/mismatch=0、public write=0、auto approval=0。 |
| `scripts/textbooks/build_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_decisions_template.js` | 从 30 个 progression action work items 生成可编辑 action decisions template；每条默认 pending，要求 reviewer 先记录 sibling context、grade-specific evidence 或 rejection reason，不能自动批准 split review。 |
| `scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_decisions.js` | 审计 progression action decisions 与 action worklist 30/30 对齐，确认 45/45 contrast rows 仍被覆盖、pending/completed=30/0、missing/extra/mismatch=0、public write=0、auto approval=0。 |
| `scripts/textbooks/build_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_evidence_packet.js` | 从 30 条 progression action decisions 生成只读 evidence packet；绑定 45 条 source contrast rows、78 个 page text excerpt previews、sibling H4G context 和 split-surface evidence by grade，供 reviewer 判断 G7/G8/G9 是否真的区分。 |
| `scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_evidence_packet.js` | 审计 progression action evidence packet 与 decisions/contrast packet 对齐，确认 expected/evidence=30/30、covered contrast=45/45、text-ready=30、missing/extra/mismatch/link=0、public write=0、auto approval=0。 |
| `scripts/textbooks/build_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_evidence_recommendations.js` | 从 progression action evidence packet 和 action decisions template 生成 recommendation-only 下一步证据路由；当前 30 条分为 sibling context repair 3、more grade-specific evidence 18、sibling-grade source evidence 9。 |
| `scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_evidence_recommendations.js` | 重新复算 action evidence recommendations，确认 recommendations/evidence=30/30、covered contrast=45/45、manual confirmation required=30、missing/extra/mismatch=0、official decision/public write/auto approval=0。 |
| `scripts/textbooks/build_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_sibling_context_repair_candidates.js` | 从 action evidence recommendations 抽取 3 条 `needs_public_sibling_progression_context_repair` P0 候选；确认它们现有 public sibling context 只有 H4G8，缺 H4G7/H4G9，必须先修 context 再重跑 contrast。 |
| `scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_sibling_context_repair_candidates.js` | 重新从 recommendations 复算 sibling-context repair candidates，确认 expected/candidates=3/3、missing/extra/mismatch=0、manual confirmation required=3、public write/auto repair/official decision=0。 |
...
| `scripts/textbooks/build_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_packet.js` | 将 52 条 source-anchor exact evidence 和 15 条 bounded-source evidence 合并为 67 条 post-candidate 统一人工复核包；保留 packet source、review lane、evidence-ready 与 pending/manual 状态，只读不写正式数据。 |
| `scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_packet.js` | 审计统一人工复核包精确覆盖 67/67 条 post-candidate 剩余项，确认 source-anchor exact 52、bounded-source 15、pending/evidence-ready/manual-required 67、missing/extra=0、auto approval=0。 |
| `scripts/textbooks/build_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_decisions_template.js` | 从 67 条统一人工复核包生成可编辑 decisions template；每条默认 pending，只允许记录后续人工确认/拒绝/需更多证据，不批准 bridge、不写 public、不启用 matcher。 |
| `scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_decisions.js` | 审计 post-candidate manual review decisions 与统一复核包 1:1 对齐；pending 默认合法，completed rows 必须保留 no-public/no-matcher/no-publication 边界并补足 reviewer confirmations。 |
| `scripts/textbooks/build_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_recommendations.js` | 从 67 条 post-candidate manual review decisions 生成 recommendation-only 分流；15 条 bounded-source 行建议进入后续确认 gate，52 条 exact-anchor 行继续 pending 人工精读，不写 public、不启用 matcher。 |
| `scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_recommendations.js` | 审计 post-candidate manual review recommendations 与 decisions template 1:1 对齐；确认 missing/extra=0、recommendation 只使用 allowed outcomes、52 条 exact-anchor 行必须保持 pending。 |
| `scripts/textbooks/build_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_confirmation_worklist.js` | 从 post-candidate manual review recommendations 中抽出 15 条 bounded-source confirmation work items，保留待填 confirmations 和 review questions；52 条 exact-anchor 行继续排除。 |
| `scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_confirmation_worklist.js` | 审计 confirmation worklist 精确覆盖 15 条 bounded-source recommendation candidates，确认 missing/extra=0、source-anchor exact rows excluded=52，且 work items 不是 decisions。 |
| `scripts/textbooks/build_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_confirmation_evidence_packet.js` | 为 15 条 bounded-source confirmation work items 抽取本地 PDF 页文本，并附带 H4G sibling grade context、review questions 和 missing confirmations；只作为后续人工 adoption 证据。 |
| `scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_confirmation_evidence_packet.js` | 审计 confirmation evidence packet 精确覆盖 15/15 条 work items，支持 `--require-text` 强制 text-extracted/ready-for-manual-review 为 15/15，且不修改 decisions、matcher 或 publication。 |
...
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_action_closure_candidates_after_post_confirmation_anchor_domain_rejected_english_pe.json` / `.md` | after-post-confirmation closure candidate；继承旧 combined 134 条候选，再加入 15 条 body-text-ready confirmation decisions，对应 source-row 7、item-level source review 8，候选闭环合计 149，剩余 52 条 exact-anchor pending。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_action_closure_candidates_after_post_confirmation_anchor_domain_rejected_english_pe_audit.json` / `.md` | after-post-confirmation closure candidate 审计；确认 expected/audited candidate closure items 为 149/149、inherited/post-confirmation 为 134/15、pending manual confirmation items 为 52、changed non-post-confirmation rows 为 0，official close-ready/auto-close 均为 0。 |
...
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_review_packet_anchor_domain_rejected_english_pe.json` / `.md` | post-candidate source-anchor exact group review packet；把 52 条 exact-anchor rows 按 subject、grade、unit、risk lane 压缩成 7 个单元级 review groups，最大单组 25 条 standards，全部只读、不可作为 approval。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_review_packet_anchor_domain_rejected_english_pe_audit.json` / `.md` | exact group review packet 审计；确认 group review items 为 7、source-anchor exact evidence items 为 52、missing/unexpected 为 0、text-extracted/ready-for-manual-review 为 52/52、auto approval 为 0。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_review_decisions_template_anchor_domain_rejected_english_pe.json` / `.md` | exact group review decisions template；把 7 个 unit-level review groups 转成 7 条 editable pending decisions，覆盖 52 条 exact-anchor evidence rows，全部 pending、auto approval 为 0。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_review_decisions_template_anchor_domain_rejected_english_pe_audit.json` / `.md` | exact group review decisions 审计；确认 expected/audited 为 7/7、missing/extra 为 0、pending/completed 为 7/0、source-anchor exact rows 为 52、groups_with_multiple_standards 为 7，仍不写 public、不启用 matcher/publication。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_review_recommendations_anchor_domain_rejected_english_pe.json` / `.md` | exact group review recommendations；7 条 recommendation-only rows 覆盖 7 条 group decisions，建议 split 4、reject overbroad/generic 2、needs evidence 1、standard-level ready 0。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_review_recommendations_anchor_domain_rejected_english_pe_audit.json` / `.md` | exact group review recommendations 审计；确认 expected/recommendations 为 7/7、missing/extra 为 0、source-anchor exact rows 为 52、standard-level ready 为 0、auto approval 为 0。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_review_action_worklist_anchor_domain_rejected_english_pe.json` / `.md` | exact group review action worklist；7 个 work items 将 recommendations 落到 split/reject/collect-evidence 三条队列，分布为 4/2/1，source-anchor exact rows 为 52。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_review_action_worklist_anchor_domain_rejected_english_pe_audit.json` / `.md` | exact group review action worklist 审计；确认 expected/action work items 为 7/7、missing/extra 为 0、standard-level ready 为 0、auto approval 为 0。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_surface_anchor_domain_rejected_english_pe.json` / `.md` | exact group split review surface；4 个 split action groups 展开为 45 条 standard-level rows，H4G7/H4G8/H4G9 为 8/34/3，unique standards 为 39。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_surface_anchor_domain_rejected_english_pe_audit.json` / `.md` | exact group split review surface 审计；确认 expected/split review items 为 45/45、parent split actions 为 4、missing/extra 为 0、standard-level ready 和 auto approval 均为 0。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_decisions_template_anchor_domain_rejected_english_pe.json` / `.md` | exact group split review decisions template；45 条 standard-level rows 全部 pending，H4G7/H4G8/H4G9 为 8/34/3，覆盖 39 个 unique standards。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_decisions_template_anchor_domain_rejected_english_pe_audit.json` / `.md` | exact group split review decisions 审计；确认 expected/decisions 为 45/45、pending/completed 为 45/0、missing/extra 为 0、auto approval 为 0。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_recommendations_anchor_domain_rejected_english_pe.json` / `.md` | exact group split review recommendations；45 条 standard-level decision rows 全部进入 recommendation-only 分流，当前仍全部 pending。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_recommendations_anchor_domain_rejected_english_pe_audit.json` / `.md` | exact group split review recommendations 审计；确认 expected/recommendations 为 45/45、missing/extra 为 0、auto approval 为 0。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_packet_anchor_domain_rejected_english_pe.json` / `.md` | exact group split review progression contrast packet；45 条 rows 中 41 条有完整 H4G7/H4G8/H4G9 sibling context 且 official standard 文本相同，25 条仍是当前年级 split evidence 孤岛，4 条缺 public sibling context。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_packet_anchor_domain_rejected_english_pe_audit.json` / `.md` | progression contrast packet 审计；确认 expected/packet 为 45/45、missing/extra/mismatch 为 0，full triplet/identical official standard 为 41/41，public write 与 auto approval 均为 0。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_worklist_anchor_domain_rejected_english_pe.json` / `.md` | progression contrast action worklist；30 个 progression-level work items 覆盖 45 条 contrast rows，P0/P1 为 3/27，work queues 为 missing sibling context 3、current-grade-specific evidence 18、sibling-grade evidence 9。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_worklist_anchor_domain_rejected_english_pe_audit.json` / `.md` | progression contrast action worklist 审计；确认 expected/action 为 30/30、covered source contrast items 为 45/45、missing/extra/mismatch 为 0、public write 与 auto approval 均为 0。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_decisions_template_anchor_domain_rejected_english_pe.json` / `.md` | progression contrast action decisions template；30 条 progression-level decision rows 全部 pending，manual confirmation required 为 30，覆盖 45/45 条 source contrast rows。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_decisions_template_anchor_domain_rejected_english_pe_audit.json` / `.md` | progression contrast action decisions 审计；确认 expected/decisions 为 30/30、pending/completed 为 30/0、missing/extra/mismatch 为 0、public write 与 auto approval 均为 0。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_evidence_packet_anchor_domain_rejected_english_pe.json` / `.md` | progression contrast action evidence packet；30 条 evidence rows 绑定 45 条 source contrast rows 和 78 个 page text excerpt previews，全部 text-ready，队列为 missing sibling context 3、current-grade evidence 18、sibling-grade evidence 9。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_evidence_packet_anchor_domain_rejected_english_pe_audit.json` / `.md` | progression action evidence packet 审计；确认 expected/evidence 为 30/30、covered source contrast items 为 45/45、missing/extra/mismatch/link 为 0、public write 与 auto approval 均为 0。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_evidence_recommendations_anchor_domain_rejected_english_pe.json` / `.md` | progression action evidence recommendations；30 条 recommendation-only rows 将已发现问题分成 needs public sibling context repair 3、needs more grade-specific source evidence 18、needs sibling-grade source evidence 9，全部要求 manual confirmation。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_evidence_recommendations_anchor_domain_rejected_english_pe_audit.json` / `.md` | progression action evidence recommendations 审计；确认 recommendations/evidence 为 30/30、source contrast 覆盖 45/45、manual confirmation required 为 30、missing/extra/mismatch 为 0、official decision/public write/auto approval 均为 0。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_sibling_context_repair_candidates_anchor_domain_rejected_english_pe.json` / `.md` | sibling-context repair candidates；3 条 P0 候选全部为 English，现有 context 为 H4G8，缺失 H4G7+H4G9，共 6 个缺失 sibling grade band instances，下一 gate 是修 public H4G sibling context 后重跑 contrast packet。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_source_anchor_exact_group_split_review_progression_contrast_action_sibling_context_repair_candidates_anchor_domain_rejected_english_pe_audit.json` / `.md` | sibling-context repair candidates 审计；确认 expected/candidates 为 3/3、missing/extra/mismatch 为 0、manual confirmation required 为 3，public write、auto repair、official decision 均为 0。 |
...
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_packet_anchor_domain_rejected_english_pe.json` / `.md` | post-candidate 统一人工复核包；67 条 items 合并 52 条 source-anchor exact evidence 与 15 条 bounded-source evidence，H4G7/H4G8/H4G9 为 23/36/8，English/PE 为 51/16，全部 pending/evidence-ready/manual-required，auto approval 为 0。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_packet_anchor_domain_rejected_english_pe_audit.json` / `.md` | 统一人工复核包审计；确认 expected/audited manual review items 为 67/67、missing/extra 为 0、source-anchor exact/bounded-source 为 52/15、pending/evidence-ready/manual-required 为 67/67/67、auto approval 为 0。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_decisions_template_anchor_domain_rejected_english_pe.json` / `.md` | post-candidate manual review decisions template；同一 67 条人工复核项全部 pending，source-anchor exact/bounded-source 为 52/15，H4G7/H4G8/H4G9 为 23/36/8，English/PE 为 51/16。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_decisions_template_anchor_domain_rejected_english_pe_audit.json` / `.md` | decisions template 审计；确认 expected/audited decisions 为 67/67、pending/completed 为 67/0、missing/extra 为 0、evidence-ready/manual-required 为 67/67，仍不写 public、不启用 matcher/publication。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_recommendations_anchor_domain_rejected_english_pe.json` / `.md` | post-candidate manual review recommendations；67 条 recommendation-only rows 覆盖同一 decisions template，7 条建议 source-row 后续确认、8 条建议 item-level source scope 后续确认、52 条 exact-anchor 继续 pending；H4G7/H4G8/H4G9 为 23/36/8，English/PE 为 51/16。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_recommendations_anchor_domain_rejected_english_pe_audit.json` / `.md` | recommendations 审计；确认 expected/recommendations 为 67/67、missing/extra 为 0、confirmation candidates 为 15、pending exact-anchor review recommendations 为 52，仍不写 public、不启用 matcher/publication。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_confirmation_worklist_anchor_domain_rejected_english_pe.json` / `.md` | post-candidate manual review confirmation worklist；15 条 work items 只覆盖 bounded-source confirmation candidates，source-row 7、item-level 8，全部 H4G7，English/PE 为 4/11；52 条 exact-anchor recommendations 被排除。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_confirmation_worklist_anchor_domain_rejected_english_pe_audit.json` / `.md` | confirmation worklist 审计；确认 expected/audited work items 为 15/15、missing/extra 为 0、exact-anchor excluded 为 52，worklist 仍不修改 editable decisions、不写 public、不启用 matcher/publication。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_confirmation_evidence_packet_anchor_domain_rejected_english_pe.json` / `.md` | post-candidate manual review confirmation evidence packet；15 条 bounded-source confirmation items 全部带 text-extracted page evidence，source-row 7、item-level 8，全部 H4G7，sibling grade context 为 H4G7+H4G8 4、H4G7+H4G8+H4G9 4、H4G7+H4G9 7。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_confirmation_evidence_packet_anchor_domain_rejected_english_pe_audit.json` / `.md` | confirmation evidence packet 审计；确认 expected/audited evidence items 为 15/15、missing/extra 为 0、text-extracted/body-text-ready/ready-for-manual-review 为 15/15，且 audit 使用 `--require-text --require-body-text`。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_confirmation_decisions_candidate_anchor_domain_rejected_english_pe.json` / `.md` | post-candidate manual review confirmation decisions candidate；把 15 条 body-text-ready bounded-source rows 标为 reviewed candidate，其中 source-row 7、item-level source scope 8，English/PE 为 4/11，全部 H4G7；52 条 source-anchor exact rows 保持 pending。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_confirmation_decisions_candidate_anchor_domain_rejected_english_pe_audit.json` / `.md` | confirmation decisions candidate 专用审计；确认 expected/candidate 为 15/15、changed non-candidate decisions 为 0、missing/extra 为 0，仍不修改源 editable template、不写 public、不启用 matcher/publication。 |
| `generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_post_candidate_manual_review_confirmation_decisions_candidate_anchor_domain_rejected_english_pe_generic_audit.json` / `.md` | confirmation decisions candidate 通过原 manual-review-decisions audit 的兼容审计；确认 completed/pending 为 15/52、evidence-ready/manual-required 为 67/67。 |
...
- `textbooks:h4g-theme-bridge-anchor-group-item-review-downstream-action-closure-candidates-after-post-confirmation` 是 post-candidate confirmation decisions 之后的后置 closure candidate gate。它不改旧 combined candidate，而是在其 134 条基础上吸收 15 条正文页确认完成的 bounded-source rows，形成 149 条 candidate closure items，剩余 52 条 exact-anchor rows 继续 pending；audit 确认 changed non-post-confirmation rows 为 0，official close/auto-close 仍为 0。
...
- `textbooks:h4g-theme-bridge-anchor-group-item-review-downstream-post-candidate-source-anchor-exact-group-review-packet` 是 exact evidence packet 后的单元级问题定位 gate。当前 52 条 exact-anchor rows 被压缩为 7 个 unit-level review groups，route 为 single-unit generic/deny-term 5 组、many-standard generic 1 组、fanout 1 组；最大单组 25 条 standards，说明 H4G7/H4G8/H4G9 仍主要卡在同一单元泛化匹配，而不是单条 standard 已经可直接通过。audit 确认 missing/unexpected 为 0，仍不写 decisions、不批准 bridge、不写正式数据。
- `textbooks:h4g-theme-bridge-anchor-group-item-review-downstream-post-candidate-source-anchor-exact-group-review-decisions` 是 exact group review packet 后的可编辑 group-level decision gate。当前 7 个 unit-level review groups 转成 7 条 pending decisions，覆盖 52 条 source-anchor exact rows，missing/extra 为 0、completed 为 0、auto approval 为 0；它只能决定下一步回到 standard-level exact-anchor review、先拆组、拒绝过宽单元或补证据，不能直接批准任何 H4G7/H4G8/H4G9 standard。
- `textbooks:h4g-theme-bridge-anchor-group-item-review-downstream-post-candidate-source-anchor-exact-group-review-recommendations` 是 exact group decisions 后的 recommendation-only 辅助 gate。当前 7 个 groups 中，4 个建议先拆组、2 个建议拒绝过宽单元/泛主题、1 个建议补更具体证据，0 个建议 standard-level ready；audit 确认 missing/extra 为 0、source-anchor exact rows 为 52、auto approval 为 0，仍不修改 editable decisions、不批准 bridge、不写正式数据。
- `textbooks:h4g-theme-bridge-anchor-group-item-review-downstream-post-candidate-source-anchor-exact-group-review-action-worklist` 是 recommendations 后的 action surface。当前 7 个 work items 精确对应 7 条 recommendations，队列 split/reject/collect evidence 为 4/2/1，missing/extra 为 0，standard-level ready 与 auto approval 均为 0；它只告诉 reviewer 下一步做什么，不修改 decisions、不写 `public/data`、不启用 matcher、不进入 publication。
- `textbooks:h4g-theme-bridge-anchor-group-item-review-downstream-post-candidate-source-anchor-exact-group-split-review-surface` 是 split action queue 后的 standard-level review surface。当前 4 个 split groups 展开成 45 条独立标准级 exact-anchor review rows，H4G7/H4G8/H4G9 为 8/34/3，unique standard codes 为 39；audit 确认 missing/extra 为 0，仍只提供 reviewer 表面，不修改 decisions、不批准任何 standard、不写正式数据。
- `textbooks:h4g-theme-bridge-anchor-group-item-review-downstream-post-candidate-source-anchor-exact-group-split-review-decisions` 是 split review surface 后的可编辑 standard-level decision gate。当前 45 条 decisions 精确对应 45 条 split review rows，全部 pending，accept candidate/reject/more evidence/activity split 均为 0，missing/extra 为 0；即便后续 accept，也只是 later action gate candidate，不是 matcher/publication approval。
- `textbooks:h4g-theme-bridge-anchor-group-item-review-downstream-post-candidate-source-anchor-exact-group-split-review-recommendations` 是 split review decisions 后的 recommendation-only 分流 gate。当前 45 条 recommendations 精确对应 45 条 split decisions，全部仍为 pending，accept/reject/more evidence/activity split 为 0，missing/extra 为 0、auto approval 为 0；它只把已完成 decision 映射到下一步候选路线，不修改 editable decisions、不批准任何 standard、不写正式数据。
- `textbooks:h4g-theme-bridge-anchor-group-item-review-downstream-post-candidate-source-anchor-exact-group-split-review-progression-contrast-packet` 是 split review 后新增的只读 progression 对照 gate。当前 45 条 contrast items 全部可复算并通过 audit；其中 41 条有完整 H4G7/H4G8/H4G9 public sibling context，且 41 条 official standard 文本完全相同；25 条只在当前年级 split surface 有 evidence，4 条只有 H4G8 public sibling context。该层明确暴露 H4G7/H4G8/H4G9 “看起来一样”的源头，但仍不修改 official standard、不写 `public/data`、不启用 matcher、不进入 publication。
- `textbooks:h4g-theme-bridge-anchor-group-item-review-downstream-post-candidate-source-anchor-exact-group-split-review-progression-contrast-action-worklist` 是 progression contrast packet 后的 action surface。它把 45 条 standard-level contrast rows 聚合成 30 个 `progression_group_id` work items，先按进阶组而不是单个 standard 判断下一步；当前队列为 repair missing sibling context 3、prove current-grade-specific evidence 18、collect sibling-grade evidence 9，覆盖 45/45 source contrast rows，missing/extra/mismatch 为 0。该层仍只是 worklist，不改 split review decisions、不批准 standard、不写正式数据。
- `textbooks:h4g-theme-bridge-anchor-group-item-review-downstream-post-candidate-source-anchor-exact-group-split-review-progression-contrast-action-decisions` 是 progression action worklist 后的人工 decision gate。当前 30 条 decisions 全部 pending，覆盖 45/45 source contrast rows；completed decision 必须带 reviewer metadata、sibling context 或 grade-specific evidence/rejection notes。该层只决定下一步证据修复状态，不改 official standard、不写正式数据、不自动批准 split review。
- `textbooks:h4g-theme-bridge-anchor-group-item-review-downstream-post-candidate-source-anchor-exact-group-split-review-progression-contrast-action-evidence-packet` 是 progression action decisions 后的只读证据包。当前 30 条 action evidence items 全部 text-ready，绑定 45/45 source contrast rows 与 78 个 page text excerpt previews；audit 确认 missing/extra/mismatch/link 为 0、public write 与 auto approval 为 0。该层只给 reviewer 判断 G7/G8/G9 的真实证据差异，不修改 editable decisions、不写 public、不启用 matcher/publication。
...
- `textbooks:h4g-theme-bridge-anchor-group-item-review-downstream-post-candidate-manual-review-packet` 是 67 条 post-candidate 剩余项的统一人工复核入口。当前合并 source-anchor exact evidence 52 条和 bounded-source evidence 15 条，H4G7/H4G8/H4G9 为 23/36/8，English/PE 为 51/16；全部 pending、evidence-ready、manual-confirmation-required，auto approval 为 0。该层只提供 reviewer packet，不修改 decisions、不写正式数据、不进入 matcher/publication。
- `textbooks:h4g-theme-bridge-anchor-group-item-review-downstream-post-candidate-manual-review-decisions` 是同一 67 条人工复核项的可编辑 decisions gate。当前 expected/audited decisions 为 67/67，pending/completed 为 67/0，missing/extra 为 0；后续即使人工填成 completed，也必须通过 confirmations 和下游 action gate 后才可能进入 matcher/publication。
- `textbooks:h4g-theme-bridge-anchor-group-item-review-downstream-post-candidate-manual-review-recommendations` 是同一 67 条 decisions 的 recommendation-only 分流 gate。当前 15 条 bounded-source rows 被建议进入后续确认，其中 source-row 7 条、item-level source scope 8 条；52 条 source-anchor exact rows 继续 pending 人工精读。audit 确认 recommendations/expected 为 67/67、missing/extra 为 0，仍不修改 editable decisions、不写正式数据、不进入 matcher/publication。
- `textbooks:h4g-theme-bridge-anchor-group-item-review-downstream-post-candidate-manual-review-confirmation-worklist` 是 15 条 bounded-source recommendation candidates 的聚焦人审入口。当前 worklist 为 15 条，source-row confirmation 7 条、item-level source scope 8 条，全部 H4G7；52 条 source-anchor exact rows 被明确排除并继续 pending。audit 确认 expected/audited 为 15/15、missing/extra 为 0；work items 不是 decisions，不修改 editable template、不写正式数据、不进入 matcher/publication。
- `textbooks:h4g-theme-bridge-anchor-group-item-review-downstream-post-candidate-manual-review-confirmation-evidence-packet` 是这 15 条 bounded-source confirmation work items 的正文页证据 gate。当前 evidence items 为 15/15，text-extracted/body-text-ready/ready-for-manual-review 为 15/15，TOC-only 为 0；page hint source 为 unit index override 4、unit index hint 2、PDF title search 2、TOC hint 后正文标题回退 7，sibling grade context 覆盖 H4G7+H4G8、H4G7+H4G9、H4G7+H4G8+H4G9。audit 使用 `--require-text --require-body-text`，仍不修改 editable decisions、不批准 bridge、不写正式数据、不进入 matcher/publication。
- `textbooks:h4g-theme-bridge-anchor-group-item-review-downstream-post-candidate-manual-review-confirmation-decisions-candidate` 是正文页证据后的非发布型 decisions candidate gate。当前 67 条 post-candidate manual review decisions 中，只有 15 条 bounded-source rows 被标为 reviewed candidate，52 条 source-anchor exact rows 保持 pending；专用 audit 确认 changed non-candidate decisions 为 0，原 manual-review-decisions audit 也通过。该层不修改源 editable template、不批准 bridge、不写正式数据、不进入 matcher/publication，只为后续 action gate 提供可审计输入。
...
- `grade7_9:h4g-differentiation-issue-matrix` 是全局 H4G 执行导航 gate。它合并 public readiness、distinctiveness audit、English/PE anchor priority matrix、anchor group decisions 和 progression action evidence recommendations，把 9 个学科分流到 anchor group decision、existing unit evidence completion、single/partial grade assignment repair、unit/chapter source indexing 或 source coverage gap；它只写 generated 报告，不写 `public/data`。
