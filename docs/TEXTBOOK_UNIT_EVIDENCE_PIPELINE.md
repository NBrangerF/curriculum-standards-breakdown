...
H4G subject theme bridge anchor recommendation 的边界：`textbooks:h4g-theme-bridge-anchor-recommendations` 读取 decisions 与 source-anchor review batch，只拒绝通过安全检查的 anchor/domain mismatch。当前默认处理 37 条：11 条 English cultural objective anchor 错打语篇、学习策略或语言知识；15 条 PE activity/movement anchor 错打课程目标、健康教育或跨学科主题学习；11 条 PE health/load-management anchor 错打课程目标、体育品德、运动能力或跨学科主题学习。新的 decisions audit 为 `valid=true`：approved 18、rejected 246、needs_revision 251、pending 0；新的 remediation packet 精确覆盖 251 条 needs_revision，新的 progression matrix 精确覆盖 58 个 source groups，仍 `complete_h4g_triplet_approved_groups=0`。
