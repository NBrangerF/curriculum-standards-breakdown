...
| --- | --- | --- | --- |
| `standard_same_grade_unit_evidence` | `standard_code` | candidate contract only | 只允许写入 `textbook_unit_evidence_ids`、`evidence_granularity`、`progression_*`、`grade_specific_focus` 等证据/复核字段；`domain/subdomain/standard/context/practice/teaching_tip/assessment_evidence_type` 不可改。 |
