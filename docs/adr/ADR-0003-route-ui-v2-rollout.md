...
通过 `RouteUiBoundary` 为 12 个生产路由建立独立 key：`home`、`subject`、`skills`、`skillDetail`、`search`、`glossary`、`standard`、`collections`、`collectionDetail`、`print`、`styleguide` 和 `feedback`。解析优先级固定为：
...
`tests/e2e/feature-flags.spec.js` 覆盖查询优先级、12 路由独立 key、冻结内容清单、增强项消失与 passthrough 内容保持。每个路由都有独立环境变量入口，不再复用其他页面的灰度状态。
