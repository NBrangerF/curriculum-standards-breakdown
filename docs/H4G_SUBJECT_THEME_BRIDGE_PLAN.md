...
3. 对 English/PE 各选一个完整 7/8/9 版本做人工/规则复核样本。
4. 新增 bridge audit：检查 no global alias、no cross-grade same-grade evidence、no unreviewed bridge、no generic term standalone。
5. 在 `match_standards_to_textbook_units.js` 中只允许 approved bridge 进入 `eligible_alignment=reviewed_subject_theme_bridge`。
6. 再补第二个教材版本，跑跨版本 consistency gate。
7. 只有通过 reviewed publication gate 后，才写入 `public/data`。
