...
4. 对 English/PE 各选一个完整 7/8/9 版本做人工/规则复核样本。
5. 新增 bridge audit：检查 no global alias、no cross-grade same-grade evidence、no unreviewed bridge、no generic term standalone。
6. 在 `match_standards_to_textbook_units.js` 中只允许通过 decisions audit 的 approved bridge 进入 `eligible_alignment=reviewed_subject_theme_bridge`。
7. 再补第二个教材版本，跑跨版本 consistency gate。
8. 只有通过 reviewed publication gate 后，才写入 `public/data`。
