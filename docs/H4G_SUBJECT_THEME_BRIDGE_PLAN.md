...
6. 新增 bridge audit：检查 no global alias、no cross-grade same-grade evidence、no unreviewed bridge、no generic term standalone。
7. 在 `match_standards_to_textbook_units.js` 中只允许通过 decisions audit 的 approved bridge 进入 `eligible_alignment=reviewed_subject_theme_bridge`。
8. 再补第二个教材版本，跑跨版本 consistency gate。
9. 只有通过 reviewed publication gate 后，才写入 `public/data`。
