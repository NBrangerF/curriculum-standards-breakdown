...
逐路由、可回滚、先内部再小流量。视觉升级不得改变内容、URL、审核流程或图谱 provenance。旧 UI 至少保留两个稳定发布周期。

## 发布顺序

1. 内部：`home`、`collections`、`glossary`、`feedback`、`print`、`h4g-review`、`styleguide`。
...
H4G 内部阶段需额外记录 390 组默认数据下的队列选择、筛选后重新定位、End/Home 长距离跳转和审核状态保存；出现空白队列、焦点落回页面或选中项与详情不一致，均按任务失败计数。
...
- Passthrough rollback：Search、Glossary、Collections、Collection Detail、Print、Style Guide、Feedback、H4G Review。flag 用于逐路由诊断和环境控制，内容/任务保持，不声称存在第二套旧页面。
...
- Chrome + VoiceOver：H4G 搜索 → 首项 End 到第 390 项 → Home 返回 → 队列选择 → 状态 → issue checkbox → 清空 Dialog 取消。
