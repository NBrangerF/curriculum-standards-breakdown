...
manifest 会记录当前请求哈希、源教材结构哈希，以及 checkpoint、decisions、alignments 三个产物的 SHA-256。apply 会重新从 checkpoint 物化结果并逐项重算稳定 ID；只接受 `complete: true`、所有 batch 成功且与当前教材、课标、能力组件和目录结构完全一致的运行。任何旧 checkpoint、产物篡改、ID 碰撞、越界页码/bbox、无效逐字证据或逻辑重复都会 fail closed。
