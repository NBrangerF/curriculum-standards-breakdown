...
function mergeTextbookRelationships(items) {
    const merged = new Map()
    for (const item of items) {
        const key = item.alignment_id
            || item.scope_id
            || [item.edition_id, item.node_id || item.unit_id || '', item.pdf_page || '', item.relation_type || ''].join(':')
        ...
        merged.set(key, {
            ...previous,
            ...item,
            evidence_spans: (item.evidence_spans || []).length >= (previous.evidence_spans || []).length ? item.evidence_spans : previous.evidence_spans,
            learning_components: (item.learning_components || []).length >= (previous.learning_components || []).length ? item.learning_components : previous.learning_components,
            learning_component_labels: (item.learning_component_labels || []).length >= (previous.learning_component_labels || []).length ? item.learning_component_labels : previous.learning_component_labels,
            learning_component_ids: (item.learning_component_ids || []).length >= (previous.learning_component_ids || []).length ? item.learning_component_ids : previous.learning_component_ids
        })
    }
...
export default function StandardTextbookLinks({ standardCode }) {
    ...
    const visibleLinks = mergeTextbookRelationships(links)
    if (visibleLinks.length === 0) return <p className={styles.loading}>尚未生成这条课标的教材关系。</p>
    ...
    return (
        <section className={styles.panel} aria-labelledby="standard-textbook-links-title">
            <header className={styles.heading}>
                ...
                <strong>{visibleLinks.filter(isSpecificLink).length} 个具体位置 · {visibleLinks.length} 条教材关系</strong>
            </header>
            <div className={styles.list}>
                {visibleLinks.map((item, index) => {
                    ...
                    const score = percentage(item.score)
                    ...
                    <article className={styles.item} key={item.alignment_id || item.scope_id || `${item.edition_id}-${index}`}>
                        ...
                        <div className={styles.actions}>
                            {item.unit_id && item.unit_assignment_status !== 'unassigned_page_only'
