    ...
    return (
        <div className={`container ${styles.page}`}>
            ...
            <div className={styles.grid}>
                <section><h2>对应课程标准</h2>{unit.alignments.length ? <div className={styles.list}>{unit.alignments.map(item => <article key={item.alignment_id}><Link to={`/standards/${encodeURIComponent(item.standard_code)}`}>{item.standard_code}</Link><p>{item.standard_text}</p><small>{item.rationale}</small></article>)}</div> : <EmptyState title="尚无已发布课标关联" description="候选关联需要通过人工复核才会出现在此处。" />}</section>
