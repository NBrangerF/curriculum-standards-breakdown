    ...
    return (
        <div className={`container ${styles.page}`}>
            ...
            <div className={styles.grid}>
                <section><h2>对应课程标准</h2>{unit.alignments.length ? <div className={styles.list}>{unit.alignments.map(item => <article key={item.alignment_id}><Link to={`/standards/${encodeURIComponent(item.standard_code)}`}>{item.standard_code}</Link><p>{item.standard_text}</p><small>{item.rationale}</small></article>)}</div> : <EmptyState title="暂无可靠的单元级关联" message="该条目尚未生成满足页内证据约束的具体关系。" />}</section>
