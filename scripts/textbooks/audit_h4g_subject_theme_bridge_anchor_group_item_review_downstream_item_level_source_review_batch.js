...
const DEFAULT_OUT = 'generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_item_level_source_review_batch_anchor_domain_rejected_english_pe_audit.json'
const DEFAULT_SUMMARY_OUT = 'generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_item_level_source_review_batch_anchor_domain_rejected_english_pe_audit.md'
...
function usage() {
  console.log(`Usage:
node scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_item_level_source_review_batch.js \\
...
Audits the downstream item-level source review batch against the downstream
...
function validateTopLevel(batch, worklist, decisions, args, errors) {
  ...
  if (batch.purpose !== 'h4g_subject_theme_bridge_anchor_group_item_review_downstream_item_level_source_review_batch') {
    errors.push('batch purpose must be h4g_subject_theme_bridge_anchor_group_item_review_downstream_item_level_source_review_batch')
  }
...
function auditReviewPolicy(item, prefix, errors) {
  ...
  const policy = item.item_level_review_policy || {}
  for (const key of [
    'downstream_decision_must_be_edited_separately',
    'item_level_source_review_batch_is_not_approval',
    'item_level_source_review_is_later_gate_input',
    'requires_later_matcher_gate',
    'requires_later_publication_gate'
  ]) {
    if (policy[key] !== true) errors.push(`${prefix} item_level_review_policy.${key} must be true`)
  }
  for (const key of [
    'writes_public_data',
    'changes_official_standard_text',
    ...
    if (policy[key] !== false) errors.push(`${prefix} item_level_review_policy.${key} must be false`)
  }
}
...
function auditItem(item, workItem, decision, errors, stats) {
  const prefix = item.item_level_source_review_item_id || item.parent_downstream_action_work_item_id || '(missing item-level source review item)'
  if (!item.item_level_source_review_item_id) errors.push(`${prefix} missing item_level_source_review_item_id`)
  auditReviewPolicy(item, prefix, errors)
  auditDecisionTemplate(item, prefix, errors)
  if (!workItem) {
    errors.push(`${prefix} item-level source review item not found in expected worklist selection`)
    return
  }
  if (!decision) {
  ...
  const checks = [
    ...
    ['standard_code', item.standard_code, workItem.standard_code || workItem.target_standard_code],
  ...
  if (item.decision_type !== 'anchor_group_downstream_item_level_source_review') {
    errors.push(`${prefix} decision_type must be anchor_group_downstream_item_level_source_review`)
  }
  if (item.source_batch !== 'child_split') errors.push(`${prefix} source_batch must be child_split`)
  if (item.source_batch_item_type !== 'child_split_review_item') {
    errors.push(`${prefix} source_batch_item_type must be child_split_review_item`)
  }
  if (Number(item.source_anchor_review_rows || 0) !== 1) errors.push(`${prefix} source_anchor_review_rows must be 1`)
  if (!arraysMatch(item.source_anchor_review_item_ids, workItem.source_anchor_review_item_ids)) {
    errors.push(`${prefix} source_anchor_review_item_ids must match source work item`)
  }
  ...
  if (workItem.source_context?.bridge_context?.page_ready !== true) {
    errors.push(`${prefix} source context must be page-ready`)
  }
  if (!item.anchor_type) errors.push(`${prefix} missing anchor_type`)
  if (!item.unit_context?.unit_evidence_id) errors.push(`${prefix} missing unit_context.unit_evidence_id`)
  ...
  for (const flag of item.risk_signals || []) countInto(stats.by_risk_signal, flag)
}
...
function markdownSummary(payload) {
  return `# H4G Downstream Item-Level Source Review Batch Audit
...
| --- | ---: |
| valid | ${payload.valid} |
| expected item-level source review items | ${payload.summary.expected_item_level_source_review_items} |
| item-level source review items | ${payload.summary.item_level_source_review_items} |
| missing review items | ${payload.summary.missing_review_items} |
| extra review items | ${payload.summary.extra_review_items} |
...
function audit(args) {
  ...
  const itemByWorkItemId = mapBy(batch.item_level_source_review_items || [], 'parent_downstream_action_work_item_id', errors, 'batch')
  const stats = {
    by_anchor_type: {},
    by_grade_band: {},
    ...
    by_source_batch: {},
    by_subject: {},
    expected_item_level_source_review_items: selected.length,
    extra_review_items: 0,
    item_level_source_review_items: (batch.item_level_source_review_items || []).length,
    missing_review_items: 0,
    parent_downstream_work_items: sorted((batch.item_level_source_review_items || []).map(item => item.parent_downstream_action_work_item_id)).length,
    source_anchor_review_rows: 0,
    unique_source_keys: sorted((batch.item_level_source_review_items || []).map(item => item.source_key)).length,
    unique_standard_codes: sorted((batch.item_level_source_review_items || []).map(item => item.standard_code)).length,
    unique_unit_evidence_ids: sorted((batch.item_level_source_review_items || []).map(item => item.unit_context?.unit_evidence_id)).length
  }
  ...
  for (const workItem of selected) {
    if (!itemByWorkItemId.has(workItem.downstream_action_work_item_id)) {
      stats.missing_review_items += 1
      errors.push(`${workItem.downstream_action_work_item_id} missing item-level source review item`)
    }
  }
  for (const item of batch.item_level_source_review_items || []) {
    if (!workItemById.has(item.parent_downstream_action_work_item_id)) stats.extra_review_items += 1
  }
  ...
  if (args.requireItems && !(batch.item_level_source_review_items || []).length) {
    errors.push('requireItems is set but batch has no item_level_source_review_items')
  }
