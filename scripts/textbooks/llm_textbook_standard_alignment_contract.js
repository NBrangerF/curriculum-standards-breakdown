...
export const LLM_ALIGNMENT_PROMPT_VERSION = 'textbook-standard-semantic-adjudicator-v1.1.0'
...
2. accept 只在教材证据直接、具体地覆盖课标的学习对象与动作要求，并且知识结构和适用情境相容时使用。每个 discovery item 包含该 logical evidence 的完整候选 scope；你必须在这整个候选集合上采用“最小充分且非冗余”原则，只接受能够由证据分别证明的最具体、最少集合。若上位课标、近义课标或重复课标没有独立要求被同一证据直接落实，必须 reject，不得为了覆盖率一并接受。
3. 必须区分同名词在不同机制中的含义，例如代数对象与几何对象、细胞代谢与生态系统能量流动、一般农业科技与航天技术、化学方程式计量与相对分子质量计算。这里只说明判别维度，不预先指定任何输入的答案。
4. 共享“性质、能量、计算、技术、活动、完成任务”等泛词、学科或单元背景、宽泛主题、情境装饰、相邻知识、仅有答案或解析结果，都不能单独构成 accept。答案页只有在同一 evidence span 同时保留了可识别的原题/任务、学习对象和目标动作时才可作为证据；否则 reject，若 OCR 或上下文残缺到无法判断则 abstain。
5. 证据不足、OCR 残缺、候选标准过宽、只能猜测具体教学意图时必须 abstain，不要为了覆盖率接受。若证据足以表明只是背景、泛化或不相关内容，则 reject，而不是 abstain。
...
6. reject 表示现有证据足以判断“不对应”；abstain 表示证据不足以可靠判断。overall_decision=abstain 时，该 item 的所有候选也必须 abstain。
7. accept 时 relation_type、evidence_level、evidence_span_id、evidence_quote 和 learning_component_ids 均由你选择。evidence_quote 必须逐字复制自所选 evidence span，learning_component_ids 必须来自该候选。L2 只表示目录/标题直接点明相同学习对象；L3 表示正文目标、讲解、活动、练习或评价任务的页内证据。teaches 不可用，因为本批次没有教师用书或官方 crosswalk 证据。
   当 unit.assignment_status=unassigned_page_only 时，该 ID 只是页窗口容器，不是正式教材单元；若 accept，必须选择 L3，不得声称已定位正式单元。
8. relation_type 必须严格按教材证据正在做的主要教学功能选择，不能按课标措辞猜测：
   - supports：正文直接定义、解释、示范或组织理解该学习对象/要求；
   - practices：学生被要求练习或实施该目标动作，且不是以检验达成为主要目的；
   - assesses：明确题目、检测或评价任务要求学生展示该目标能力；答案/解析页本身不算 assesses；
...
9. reject/abstain 时 relation_type、evidence_level、evidence_span_id 必须为 null，evidence_quote 必须为空字符串，learning_component_ids 必须为空数组。
10. rationale 必须说明教材证据与课标对象/动作/机制为什么相容、不相容或为何不足；accept 还必须说明为何所选 relation_type 是该证据的主要教学功能，以及为何不是更宽泛或重复候选。不得使用“相似度高”“系统判定”等空泛表述，不得生成数值分数或 confidence。
11. 每个输入 candidate 必须恰好返回一次，candidate_id、standard_code、item_id 必须原样返回。只返回符合 JSON Schema 的对象。`
