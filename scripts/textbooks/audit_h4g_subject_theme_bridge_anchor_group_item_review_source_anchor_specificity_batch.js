...
const DEFAULT_BATCH = 'generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_source_anchor_specificity_batch_anchor_domain_rejected_english_pe.json'
...
const DEFAULT_OUT = 'generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_source_anchor_specificity_batch_anchor_domain_rejected_english_pe_audit.json'
const DEFAULT_SUMMARY_OUT = 'generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_source_anchor_specificity_batch_anchor_domain_rejected_english_pe_audit.md'
...
function usage() {
  console.log(`Usage:
node scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_source_anchor_specificity_batch.js \\
  --batch generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_source_anchor_specificity_batch_anchor_domain_rejected_english_pe.json \\
...
Audits the H4G source-anchor specificity batch against the item-review action
...
function validateTopLevel(batch, worklist, splitBatch, args, errors) {
  ...
  if (batch.purpose !== 'h4g_subject_theme_bridge_anchor_group_item_review_source_anchor_specificity_batch') {
    errors.push('batch purpose must be h4g_subject_theme_bridge_anchor_group_item_review_source_anchor_specificity_batch')
  }
...
function auditPolicy(item, prefix, errors) {
  ...
  for (const key of [
    ...
    'read_only_source_anchor_specificity_batch',
    'requires_later_matcher_gate',
    'requires_later_publication_gate',
    'source_anchor_specificity_review_is_not_approval',
...
function auditDecisionTemplate(item, prefix, errors) {
  ...
  if (confirmations.source_anchor_is_specific_not_topic_only !== false) {
    errors.push(`${prefix} source_anchor_is_specific_not_topic_only must start false`)
  }
...
function auditItem(item, expected, errors, stats) {
  const prefix = item.source_anchor_specificity_review_item_id || item.parent_action_work_item_id || '(missing specificity item)'
  if (!item.source_anchor_specificity_review_item_id) errors.push(`${prefix} missing source_anchor_specificity_review_item_id`)
  auditPolicy(item, prefix, errors)
  auditDecisionTemplate(item, prefix, errors)
  if (!expected) {
    errors.push(`${prefix} specificity item not found in expected worklist selection`)
    return
  }
  ...
  if (item.decision_type !== 'anchor_group_source_anchor_specificity_review') {
    errors.push(`${prefix} decision_type must be anchor_group_source_anchor_specificity_review`)
  }
  if (item.item_review_surface !== 'anchor_group_source_anchor_specificity_review') {
    errors.push(`${prefix} item_review_surface must be anchor_group_source_anchor_specificity_review`)
  }
...
function auditSummary(batch, stats, errors) {
  ...
  if (summary.source_anchor_specificity_review_items !== stats.source_anchor_specificity_review_items) {
    errors.push('summary.source_anchor_specificity_review_items mismatch')
  }
...
function markdownSummary(payload) {
  return `# H4G Anchor Group Source-Anchor Specificity Batch Audit
...
| --- | ---: |
| valid | ${payload.valid} |
| expected specificity items | ${payload.summary.expected_source_anchor_specificity_review_items} |
| specificity review items | ${payload.summary.source_anchor_specificity_review_items} |
| missing specificity items | ${payload.summary.missing_source_anchor_specificity_review_items} |
| extra specificity items | ${payload.summary.extra_source_anchor_specificity_review_items} |
...
function audit(args) {
  ...
  const batch = errors.length ? { source_anchor_specificity_review_items: [] } : readJson(args.batch)
  ...
  const expectedByKey = mapBy(expected, 'expected_id_key', errors, 'expected specificity rows')
  ...
  const stats = {
    ...
    source_anchor_specificity_review_items: actualRows.length,
    source_keys: new Set()
  }
  ...
  for (const row of expected) {
    if (!actualByKey.has(row.expected_id_key)) {
      ...
      errors.push(`${row.expected_id_key} missing specificity review item`)
    }
  }
  for (const item of actualRows) {
  ...
  if (args.requireItems && !expected.length) errors.push('requireItems is set but expected specificity rows are empty')
  if (args.requireItems && !actualRows.length) {
    errors.push('requireItems is set but batch has no source_anchor_specificity_review_items')
  }
  ...
  return {
    ...
    summary: {
      ...
      expected_source_anchor_specificity_review_items: stats.expected_source_anchor_specificity_review_items,
      extra_source_anchor_specificity_review_items: stats.extra_source_anchor_specificity_review_items,
      missing_source_anchor_specificity_review_items: stats.missing_source_anchor_specificity_review_items,
      parent_work_items: stats.parent_work_item_ids.size,
      source_anchor_review_rows: stats.source_anchor_review_rows,
      source_anchor_specificity_review_items: stats.source_anchor_specificity_review_items,
