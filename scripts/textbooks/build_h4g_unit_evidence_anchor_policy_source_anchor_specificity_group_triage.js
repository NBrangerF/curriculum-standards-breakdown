...
const DEFAULT_OUT = 'generated/textbook_evidence/h4g_unit_evidence_anchor_policy_source_anchor_specificity_group_triage.json'
const DEFAULT_SUMMARY_OUT = 'generated/textbook_evidence/h4g_unit_evidence_anchor_policy_source_anchor_specificity_group_triage.md'
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    const item = argv[i]
    if (item === '--evidence-packet') args.evidencePacket = argv[++i]
...
function usage() {
  console.log(`Usage:
node scripts/textbooks/build_h4g_unit_evidence_anchor_policy_source_anchor_specificity_group_triage.js \\
  --strict --require-items

Builds a read-only group triage from H4G unit source-anchor specificity evidence
packets. It separates ready evidence groups from missing-anchor and partial
context repair groups. It does not edit decisions, write public/data, change
...
function validateEvidencePacket(packet, errors) {
  ...
  if (packet.evidence_packet_only !== true) errors.push('evidence packet evidence_packet_only must be true')
  ...
  for (const key of [
    ...
    if (packet[key] !== false) errors.push(`evidence packet ${key} must be false`)
  }
}
...
function buildTriageItem(item) {
  const route = triageRoute(item)
  const standardGradeBands = sorted(item.grade_bands_in_standard_group || [])
  ...
  return {
    ...
    standard_grade_bands: standardGradeBands,
    subject_slug: item.subject_slug || '',
    target_grade_bands: TARGET_GRADE_BANDS,
    triage_route: route,
    triage_scope_note: 'Group-level routing only. It is not a source-anchor decision and must not be copied into public standards.',
    writes_public_data: false
  }
}
...
function triageRowsTable(rows) {
  return rows.map(row => (
    `| ${row.priority} | ${markdownCell(row.progression_group_id)} | ${markdownCell(row.subject_slug)} | ${markdownCell(row.triage_route)} | ${markdownCell(row.anchor_grade_bands.join(', '))} | ${markdownCell(row.missing_target_anchor_grade_bands.join(', ') || 'none')} | ${row.anchor_summary.anchor_rows} |`
  )).join('\n') || '| - | - | - | - | - | - | 0 |'
}

function markdownSummary(payload) {
  return `# H4G Unit Source-Anchor Specificity Group Triage

Generated at: ${payload.generated_at}

This read-only triage routes source-anchor evidence groups by data quality. It
does not edit decisions, write \`public/data\`, change official standard text,
...
| --- | ---: |
| valid | ${payload.valid} |
| group triage items | ${payload.summary.group_triage_items} |
...
| triage anchor rows | ${payload.summary.triage_anchor_rows} |
| triage standard context rows | ${payload.summary.triage_standard_context_rows} |
...
| priority | progression group | subject | route | anchor grades | missing anchor grades | anchor rows |
| ---: | --- | --- | --- | --- | --- | ---: |
${triageRowsTable(payload.source_anchor_specificity_group_triage_items)}
...
function buildPayload(args) {
  const errors = []
  if (!existsSync(args.evidencePacket)) errors.push(`Missing evidence packet: ${args.evidencePacket}`)
  const packet = existsSync(args.evidencePacket) ? readJson(args.evidencePacket) : { source_anchor_specificity_evidence_items: [] }
  if (!errors.length) validateEvidencePacket(packet, errors)
  const rows = (packet.source_anchor_specificity_evidence_items || [])
    .map(buildTriageItem)
  ...
  if (args.requireItems && !rows.length) errors.push('requireItems is set but no group triage items were generated')
  return {
    changes_official_standard_text: false,
    direct_matcher_use: false,
    eligible_for_h4g_differentiation: false,
    ...
    policy: policy(),
    publication_ready: false,
    purpose: 'h4g_unit_evidence_anchor_policy_source_anchor_specificity_group_triage',
    requires_later_anchor_policy_decision_edit: true,
    requires_later_candidate_coverage_recheck: true,
    requires_later_consistency_gate: true,
    requires_later_manual_review: true,
    requires_later_publication_gate: true,
    review_batch_only: true,
    source_anchor_policy_source_anchor_specificity_evidence_packet: args.evidencePacket,
    source_anchor_specificity_batch_only: true,
    source_anchor_specificity_group_triage_items: rows,
    source_anchor_specificity_group_triage_only: true,
