...
const DEFAULT_OUT = 'generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_source_row_confirmation_batch_anchor_domain_rejected_english_pe_audit.json'
const DEFAULT_SUMMARY_OUT = 'generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_source_row_confirmation_batch_anchor_domain_rejected_english_pe_audit.md'
...
function usage() {
  console.log(`Usage:
node scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_source_row_confirmation_batch.js \\
...
Audits the downstream source-row confirmation batch against the downstream
...
function validateTopLevel(batch, worklist, decisions, args, errors) {
  ...
  if (batch.purpose !== 'h4g_subject_theme_bridge_anchor_group_item_review_downstream_source_row_confirmation_batch') {
    errors.push('batch purpose must be h4g_subject_theme_bridge_anchor_group_item_review_downstream_source_row_confirmation_batch')
  }
...
function auditConfirmationPolicy(item, prefix, errors) {
  ...
  const policy = item.confirmation_policy || {}
  for (const key of [
    'downstream_decision_must_be_edited_separately',
    'requires_later_matcher_gate',
    'requires_later_publication_gate',
    'source_row_confirmation_batch_is_not_approval',
    'source_row_confirmation_is_later_gate_input'
  ]) {
    if (policy[key] !== true) errors.push(`${prefix} confirmation_policy.${key} must be true`)
  }
  for (const key of [
    'writes_public_data',
    'changes_official_standard_text',
    ...
    if (policy[key] !== false) errors.push(`${prefix} confirmation_policy.${key} must be false`)
  }
}

function auditDecisionTemplate(item, prefix, errors) {
  const template = item.confirmation_decision_template || {}
  const allowed = template.allowed_confirmation_outcomes || []
  for (const value of [
    ...
    if (!allowed.includes(value)) errors.push(`${prefix} allowed_confirmation_outcomes missing ${value}`)
  }
  for (const forbidden of ['matcher_ready', 'publication_ready', 'approve_standard_scoped_subject_theme_bridge']) {
    if (allowed.includes(forbidden)) errors.push(`${prefix} confirmation template must not include ${forbidden}`)
  }
...
function auditItem(item, workItem, decision, errors, stats) {
  const prefix = item.source_row_confirmation_item_id || item.parent_downstream_action_work_item_id || '(missing source row confirmation item)'
  if (!item.source_row_confirmation_item_id) errors.push(`${prefix} missing source_row_confirmation_item_id`)
  auditConfirmationPolicy(item, prefix, errors)
  auditDecisionTemplate(item, prefix, errors)
  if (!workItem) {
    errors.push(`${prefix} source row confirmation item not found in expected worklist selection`)
    return
  }
  if (!decision) {
  ...
  if (item.decision_type !== 'anchor_group_downstream_source_row_confirmation') {
    errors.push(`${prefix} decision_type must be anchor_group_downstream_source_row_confirmation`)
  }
  ...
  countInto(stats.by_recommendation_confidence, item.recommendation_confidence)
...
function markdownSummary(payload) {
  return `# H4G Downstream Source Row Confirmation Batch Audit
...
| --- | ---: |
| valid | ${payload.valid} |
| expected source row confirmations | ${payload.summary.expected_source_row_confirmations} |
| source row confirmation items | ${payload.summary.source_row_confirmation_items} |
| missing confirmation items | ${payload.summary.missing_confirmation_items} |
| extra confirmation items | ${payload.summary.extra_confirmation_items} |
...
function audit(args) {
  ...
  const itemByWorkItemId = mapBy(batch.source_row_confirmation_items || [], 'parent_downstream_action_work_item_id', errors, 'batch')
  const stats = {
    by_anchor_type: {},
    by_grade_band: {},
    ...
    by_source_batch: {},
    by_subject: {},
    expected_source_row_confirmations: selected.length,
    extra_confirmation_items: 0,
    missing_confirmation_items: 0,
    parent_downstream_work_items: sorted((batch.source_row_confirmation_items || []).map(item => item.parent_downstream_action_work_item_id)).length,
    source_anchor_review_rows: 0,
    source_row_confirmation_items: (batch.source_row_confirmation_items || []).length,
    unique_source_keys: sorted((batch.source_row_confirmation_items || []).map(item => item.source_key)).length
  }
  ...
  for (const workItem of selected) {
    if (!itemByWorkItemId.has(workItem.downstream_action_work_item_id)) {
      stats.missing_confirmation_items += 1
      errors.push(`${workItem.downstream_action_work_item_id} missing source row confirmation item`)
    }
  }
  for (const item of batch.source_row_confirmation_items || []) {
    if (!workItemById.has(item.parent_downstream_action_work_item_id)) stats.extra_confirmation_items += 1
  }
  ...
  if (args.requireItems && !(batch.source_row_confirmation_items || []).length) {
    errors.push('requireItems is set but batch has no source_row_confirmation_items')
  }
