...
const DEFAULT_OUT = 'generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_action_recommendations_anchor_domain_rejected_english_pe_audit.json'
const DEFAULT_SUMMARY_OUT = 'generated/textbook_evidence/h4g_theme_bridge_anchor_group_item_review_downstream_action_recommendations_anchor_domain_rejected_english_pe_audit.md'
...
function usage() {
  console.log(`Usage:
node scripts/textbooks/audit_h4g_subject_theme_bridge_anchor_group_item_review_downstream_action_recommendations.js \\
...
Audits recommendation-only downstream action-review routing against the editable
downstream action decisions template.`)
}
...
function auditRecommendation(row, decision, errors, stats) {
  const prefix = row.decision_id || row.source_downstream_action_item_id || '(missing downstream action recommendation)'
  ...
  if (row.action_recommendation_decision_id !== row.decision_id) {
  ...
  const checks = [
    ...
    ['source_downstream_action_item_type', row.source_downstream_action_item_type, decision.source_downstream_action_item_type],
    ['source_key', row.source_key, decision.source_key],
    ['standard_code', row.standard_code, decision.standard_code],
    ['source_standard_code', row.source_standard_code, decision.source_standard_code],
  ...
  for (const [field, actual, expected] of checks) {
    if (String(actual ?? '') !== String(expected ?? '')) errors.push(`${prefix} ${field} must match downstream action decisions template`)
  }
  if (!Array.isArray(row.allowed_decisions) || !row.allowed_decisions.includes(row.recommended_reviewer_decision)) {
    errors.push(`${prefix} recommended_reviewer_decision must be allowed by the source action decision`)
  }
  if (row.recommended_reviewer_decision !== decision.recommended_reviewer_decision) {
    errors.push(`${prefix} recommended_reviewer_decision must match inherited source action recommendation`)
  }
...
function auditSummary(recommendations, stats, errors) {
  ...
  if (summary.downstream_action_recommendations !== stats.downstream_action_recommendations) {
    errors.push('summary.downstream_action_recommendations mismatch')
  }
  if (summary.source_anchor_review_rows !== stats.source_anchor_review_rows) {
    errors.push('summary.source_anchor_review_rows mismatch')
  }
}

function markdownSummary(payload) {
  return `# H4G Downstream Action Recommendations Audit
...
function audit(args) {
  ...
  const recByDecisionId = mapBy(recommendations.downstream_action_recommendations || [], 'decision_id', errors, 'recommendations')
  const stats = {
    by_grade_band: {},
    by_recommendation: {},
    ...
    by_subject: {},
    downstream_action_recommendations: (recommendations.downstream_action_recommendations || []).length,
  ...
  for (const row of recommendations.downstream_action_recommendations || []) {
  ...
  for (const decision of decisions.downstream_action_decisions || []) {
    if (!recByDecisionId.has(decision.decision_id)) {
      ...
      errors.push(`${decision.decision_id} missing downstream action recommendation`)
    }
  }
  for (const row of recommendations.downstream_action_recommendations || []) {
