...
test('GET /api/v1/openapi.yaml and /api/v1/docs expose API documentation', async () => {
    ...
    assert.match(spec, /SmartSearchQueryPlan:/)
    assert.match(spec, /enum: \[nlq-v2\]/)
    assert.match(spec, /understanding_summary:/)
    assert.match(spec, /query: 第二学段 G3-4，和阅读相关的标准，但不是语文学科中的/)
...
test('POST /api/v1/standards/semantic-search returns trusted explainable candidates', async () => {
    ...
    assert.deepEqual(body.data.query_interpretation.constraint_evidence, [])
    assert.deepEqual(body.data.query_interpretation.ambiguities, [])
    assert.equal(body.data.query_interpretation.clarification_question, '')
    assert.equal(body.meta.query_interpreter, 'deterministic_fallback')
    assert.equal(body.data.query_plan.version, 'nlq-v2')
    assert.equal(typeof body.data.understanding_summary, 'string')
